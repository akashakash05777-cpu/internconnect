...
export default function LoginPage() {
  ...
  useEffect(() => {
    const storedEmail = typeof window !== 'undefined' ? localStorage.getItem('ic_email') : null;
    if (storedEmail) {
  ...
  useEffect(() => {
    ...
    const score = [hasLength, hasNumber, hasSymbol, hasCase].filter(Boolean).length;
    setPasswordStrength(score >= 4 ? 'strong' : score >= 2 ? 'medium' : 'weak');
  }, [password]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      if (rememberMe) {
        localStorage.setItem('ic_email', email);
      } else {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center p-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div
                        className={
                          passwordStrength === 'strong'
                            ? 'h-1 w-full rounded bg-green-500'
                            : passwordStrength === 'medium'
                            ? 'h-1 w-2/3 rounded bg-yellow-500'
                            : 'h-1 w-1/3 rounded bg-red-500'
                        }
                      />
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Switch checked={rememberMe} onCheckedChange={(v) => setRememberMe(Boolean(v))} id="remember" />
                    <Label htmlFor="remember" className="text-sm text-muted-foreground">Remember me</Label>
                  </div>
                  <Button type="button" variant="link" className="px-0 text-sm" onClick={() => toast.info('Use the demo credentials on the right to sign in.')}>Forgot password?</Button>
                </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              navigator.clipboard.writeText(`${demo.email} / ${demo.password}`);
